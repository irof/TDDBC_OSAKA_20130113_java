...
public class VendingMachineTest {
    ...
    public static class お金が投入されていない状態 {
        ...
        @Test
		public void お金を投入していない状態で払い戻し操作を行うと０円が帰ってくる() throws Exception {
        	sut.payback();
    ...
    public static class お金が投入されている状態 {
        ...
        @Test
        public void 払い戻すと投入金額が0円になる() {
            sut.payback();
            assertThat(sut.getCreditAmount(), is(0));
        }
        
        @Test
		public void 在庫が1で購入すると0になる() throws Exception {
        	在庫を0にする();
			assertThat(sut.getStockText(),is("コーラ:120円:0本"));
		}

        @Test
		public void 在庫が0のときに購入しても在庫は減らない() throws Exception {
        	在庫を0にする();
        	sut.purchase();
			assertThat(sut.getStockText(),is("コーラ:120円:0本"));
		}
    ...
    public static class 金種による扱い {
        ...
        @Theory
        public void 扱えないお金を投入_釣りになる(@TestedOn(ints = {1, 5, 2000, 5000, 10000}) int amount) {
            sut.insert(amount);
            assertThat(sut.getChangeAmount(), is(amount));
        }
    }
}
